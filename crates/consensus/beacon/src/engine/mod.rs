...
//     /// Sends a forkchoice update message to the beacon consensus engine and waits for a
// response.     ///
...
//     /// Sends a forkchoice update message to the beacon consensus engine and returns the receiver
...
// /// [BeaconEngineMessage::ForkchoiceUpdated] message from the CL which would initiate the sync.
// At /// first, the consensus engine would run the [Pipeline] until the latest known block hash.
// /// Afterwards, it would attempt to create/restore the [`BlockchainTreeEngine`] from the blocks
// /// that are currently available. In case the restoration is successful, the consensus engine
// would /// run in a live sync mode, which mean it would solemnly rely on the messages from Engine
// API to /// construct the chain forward.
// ///
// /// # Panics
// ///
...
//     /// If true, the engine will run the pipeline continuously, regardless of whether or not
// there     /// is a new fork choice state.
//     continuous: bool,
//     /// The payload store.
...
//     /// Called to resolve chain forks and ensure that the Execution layer is working with the
// latest     /// valid chain.
//     ///
...
//                     debug!(target: "consensus::engine", hash=?state.head_block_hash,
// number=head_block_number, "canonicalized new head");

//                     let pipeline_min_progress =
//                         FINISH.get_progress(&self.db.tx()?)?.unwrap_or_default();

//                     if pipeline_min_progress < head_block_number {
//                         debug!(target: "consensus::engine", last_finished=pipeline_min_progress,
// head_number=head_block_number, "pipeline run to head required");                         
...
//                 }
//                 Err(error) => {
//                     warn!(target: "consensus::engine", ?error, ?state, "Error canonicalizing the
// head hash");                     // If this is the first forkchoice received, start downloading
// from safe block                     // hash.
...
//             }
//         } else {
//             trace!(target: "consensus::engine", "Pipeline is syncing, skipping forkchoice
// update");             PayloadStatus::from_status(PayloadStatusEnum::Syncing)
//         };
...
//             Ok(block) => block,
//             Err(error) => {
//                 error!(target: "consensus::engine", ?block_hash, block_number, ?error, "Invalid
// payload");                 return error.into()
//             }
//         };
...
//         };
//         trace!(target: "consensus::engine", ?block_hash, block_number, ?status, "Returning
...
//             trace!(target: "consensus::engine", ?tip, continuous = tip.is_none(), "Starting the
// pipeline");             let (tx, rx) = oneshot::channel();
...
//                             Err(error) => {
//                                 error!(target: "consensus::engine", ?state, ?error, "Error
// getting forkchoice updated response");                                 return
...
//                                     } else {
//                                         // Terminate the sync early if it's reached the maximum
// user                                         // configured block.
...
//                                 error!(target: "consensus::engine", ?error, "Error restoring
// blockchain tree");                                 return Poll::Ready(Err(error.into()))
//                             }
...
//                         }
//                         Poll::Ready(Err(error)) => {
//                             error!(target: "consensus::engine", ?error, "Failed to receive
// pipeline result");                             return
// Poll::Ready(Err(BeaconEngineError::PipelineChannelClosed))                         }
//                         Poll::Pending => {
...
//         /// Sends the `ExecutionPayload` message to the consensus engine and retries if the
// engine         /// is syncing.
...
//         /// Sends the `ForkchoiceUpdated` message to the consensus engine and retries if the
// engine         /// is syncing.
...
//         assert_matches!(
//             res.await,
//             Ok(Err(BeaconEngineError::Pipeline(n))) if
// matches!(*n.as_ref(),PipelineError::Stage(StageError::ChannelClosed))         );
//     }
...
//     fn insert_blocks<'a, DB: Database>(db: &DB, mut blocks: impl Iterator<Item = &'a
// SealedBlock>) {         let mut transaction = Transaction::new(db).unwrap();
//         blocks
//             .try_for_each(|b| {
//                 transaction
//                     .insert_block(SealedBlockWithSenders::new(b.clone(),
// Vec::default()).unwrap())             })
...
//             let result =
// env.send_forkchoice_retry_on_syncing(next_forkchoice_state).await.unwrap();             let
// expected_result = ForkchoiceUpdated::from_status(PayloadStatusEnum::Valid)                 
// .with_latest_valid_hash(next_head.hash);             assert_eq!(result, expected_result);
...
//             assert_matches!(res, Ok(result) => assert_matches!(result.status,
...
//         //         validation_error: ExecutorError::BlockPreMerge { hash: block2.hash
// }.to_string(),         //     })
//         //     .with_latest_valid_hash(H256::zero());
//         //     assert_eq!(result, expected_result);
