...
pub trait StaticValidity: Debug + Send + Sync {
    /// Validate if header is correct and follows consensus specification.
    ///
    /// This is called on standalone header to check if all hashe
    fn validate_header(&self, header: &SealedHeader) -> Result<(), StaticValidityError>;
    ...
    fn validate_block(&self, block: &SealedBlock) -> Result<(), StaticValidityError>;
}

/// Consensus Errors
#[allow(missing_docs)]
#[derive(thiserror::Error, Debug, PartialEq, Eq, Clone)]
pub enum StaticValidityError {
