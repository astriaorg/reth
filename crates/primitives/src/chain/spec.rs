...
pub static MAINNET: Lazy<Arc<ChainSpec>> = Lazy::new(|| {
    ChainSpec {
        ...
        hardforks: BTreeMap::from([
            (Hardfork::Acapella, ForkCondition::Block(0)),
        ]),
    }
    .into()
});

/// The Symphony devnet spec
...
impl ChainSpec {
    ...
    pub fn genesis_header(&self) -> Header {
        ...
        let withdrawals_root =
            (self.fork(Hardfork::Acapella).active_at_timestamp(self.genesis.timestamp))
    ...
    pub fn initial_base_fee(&self) -> Option<u64> {
        // If London is activated at genesis, we set the initial base fee as per EIP-1559.
        (self.fork(Hardfork::Acapella).active_at_block(0)).then_some(EIP1559_INITIAL_BASE_FEE)
    }
    ...
    /// Convenience method to check if [Hardfork::Acapella] is active at a given timestamp.
    #[inline]
    pub fn is_shanghai_activated_at_timestamp(&self, timestamp: u64) -> bool {
        self.fork_timestamps
            .shanghai
            .map(|shanghai| timestamp >= shanghai)
            .unwrap_or_else(|| self.is_fork_active_at_timestamp(Hardfork::Acapella, timestamp))
    }
...
impl ForkTimestamps {
    ...
    fn from_hardforks(forks: &BTreeMap<Hardfork, ForkCondition>) -> Self {
        ...
        if let Some(shanghai) = forks.get(&Hardfork::Acapella).and_then(|f| f.as_timestamp()) {
...
impl ChainSpecBuilder {
    ...
    /// Enable Acapella at genesis.
    pub fn frontier_activated(mut self) -> Self {
        self.hardforks.insert(Hardfork::Acapella, ForkCondition::Block(0));
        self
    }
