...
// #[add_arbitrary_tests(rlp)] // FIXME: what do we need to do to add this back?
...
impl From<u64> for Chain {
    fn from(id: u64) -> Self {
        SymphonyChains::try_from(id).map(Chain::Named).unwrap_or_else(|_| Chain::Id(id))
    }
}
...
impl Encodable for Chain {
    fn encode(&self, out: &mut dyn reth_rlp::BufMut) {
        match self {
            Self::Named(chain) => u64::try_from(*chain).unwrap(),
            Self::Id(id) => *id,
        }.encode(out)
    }
    ...
    fn length(&self) -> usize {
        match self {
            Self::Named(chain) => u64::try_from(*chain).unwrap().length(),
...
#[cfg(test)]
mod tests {
    use symphony_primitives::constants::DEVNET_ID;
    ...
    fn test_into_u256() {
        ...
        let expected = U256::from(DEVNET_ID);
